...
def main():
    ...
    if os.path.exists("./train_temp.csv"):
        ...
        print(f"elapsed time - retriever : {time.time() - start_time:.2f}s. "
              f"i.e. {(time.time() - start_time)//60}min and {(time.time() - start_time)%60:.2f}s")
    ...
    s2s_trainer = Trainer(logger=tb_logger, precision=32, gpus=gpus,
                          accumulate_grad_batches=accumulation_gradient, max_epochs=50,
    ...
    print(f"elapsed time - genrator : {time.time()-start_time_generator:.2f}s. "
              f"i.e. {(time.time() - start_time_generator)//60}min and {(time.time()-start_time_generator)%60:.2f}s")
