...
def get_datasets(
    ...
    if "all" in text_column:
        df_train.rename(columns={"text_all_passage": text_column}, inplace=True)
    elif 'first' in text_column:
        df_train.rename(columns={"text_first_sentence": text_column}, inplace=True)
    ...
    if "all" in text_column:
        df_val.rename(columns={"text_all_passage": text_column}, inplace=True)
    elif 'first' in text_column:
        df_val.rename(columns={"text_first_sentence": text_column}, inplace=True)
    corpus_val = CorpusDataset(path_to_file=val_corpus, text_column=text_column)
    df_test = pd.read_json(test_ds)
    if "all" in text_column:
        df_test.rename(columns={"text_all_passage": text_column}, inplace=True)
    elif 'first' in text_column:
        df_test.rename(columns={"text_first_sentence": text_column}, inplace=True)
...
def check_cohenrence(text_column,
    ...
    only_skipped_2 = ("skipped" in train_ds) and ("skipped" in val_ds) and ("skipped" in test_ds)
...
@click.option("--test-corpus", default="test/corpus_test.json",
              help="test corpus.")
@click.option("--nb-passages", default=None, type=int,
              help="if custom test_ds, nb of passages to use from corpus")
@click.option("--test-prefix", default=True,
              help="if specific test file, then no prefix")
def main(
        text_column,
        checkpoints_suffix,
        suffix,
